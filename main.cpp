...
int main(int argc, char** argv) {
    ...
    opmode.add_argument("-r", "--read").help("Use read mode(exclusive with --write)").flag().store_into(mode_read);
    opmode.add_argument("-w", "--write").help("Use write mode(exclusive with --read)").flag().store_into(mode_write);
    args.add_argument("address")
    .help("The address to read/write memory to")
    .scan<'x', uint64>()
    .store_into(addr);
    ...
    .default_value(0)
    .store_into(size);
    args.add_argument("-f", "--file").help("Name of file to read/write dump to(defaults to stdin/stdout if unspecified)").store_into(filename);
    args.add_argument("-p", "--pid").help("PID of process to read/write memory from").required().scan<'u', uint64>().store_into(pid);
